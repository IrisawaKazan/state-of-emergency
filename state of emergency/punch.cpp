...
// コンストラクタ
//----------------------------------------
CPunch::CPunch(int nPriority) : CObject(nPriority)
{
	m_pTexture = nullptr;
	m_pMesh = nullptr;
	m_pBuffMat = nullptr;
...
// 生成処理
//----------------------------------------
CPunch* CPunch::Create(D3DXVECTOR3 pos, D3DXVECTOR3 rot, PUNCH type)
{
	CPunch* pPunch;
	...
	if (nNumAll <= MAX_OBJ)
	{
		...
		// 位置の設定
		pPunch->SetPosition(pos);

		// 向きの設定
		pPunch->SetRotation(rot);
...
HRESULT CPunch::Init(void)
{
	...
	// Xファイルの読み込み
	D3DXLoadMeshFromX("data\\MODEL\\punch.x",
		D3DXMESH_SYSTEMMEM,
		pDevice,
		NULL,
		&m_pBuffMat,
...
void CPunch::Update(void)
{
	...
	case PUNCH_000_A:
		...
		// パンチの動き
		if (m_nMovingCounter[0] >= 60 * 0 && m_nMovingCounter[0] < 60 * 8) // 0秒から8秒の間は
		{
			// 動かない
		}
		if (m_nMovingCounter[0] >= 60 * 8 && m_nMovingCounter[0] < 60 * 10) // 8秒から10秒の間は
		{
			// 少し勢いをつけるように動く
			m_pos.x += 0.1f;
			m_pos.z += 0.1f;
		}
		if (m_nMovingCounter[0] >= 60 * 10 && m_nMovingCounter[0] < 60 * 11 - 50) // 10秒から11秒と5/6秒の間は
		{
			// 一気に進む
			m_pos.x -= 30.0f;
			m_pos.z -= 30.0f;
		}
		if (m_nMovingCounter[0] >= 60 * 11 - 50 && m_nMovingCounter[0] < 60 * 12) // 11秒と5/6秒から12秒の間は
		{
			// 動かない
		}
		if (m_nMovingCounter[0] >= 60 * 12) // 13秒に
		{
			// ワープして戻る
	...
	case PUNCH_000_C:
		...
		// パンチの動き
		if (m_nMovingCounter[2] >= 60 * 0 && m_nMovingCounter[2] < 60 * 15) // 0秒から15秒の間は
		{
			// 動かない
		}
		if (m_nMovingCounter[2] >= 60 * 15 && m_nMovingCounter[2] < 60 * 17) // 15秒から17秒の間は
		{
			// 少し勢いをつけるように動く
			m_pos.x += 0.1f;
			m_pos.z -= 0.1f;
		}
		if (m_nMovingCounter[2] >= 60 * 17 && m_nMovingCounter[2] < 60 * 18 - 50) // 17秒から18秒と5/6秒の間は
		{
			// 一気に進む
			m_pos.x -= 30.0f;
			m_pos.z += 30.0f;
		}
		if (m_nMovingCounter[2] >= 60 * 15 - 50 && m_nMovingCounter[2] < 60 * 16) // 18秒と5/6秒から19秒の間は
		{
			// 動かない
		}
		if (m_nMovingCounter[2] >= 60 * 19) // 19秒に
		{
			// ワープして戻る
			m_pos = D3DXVECTOR3(350.0f, 0.0f, -250.0f);

			// カウンターリセット
			m_nMovingCounter[2] = /*0*/60 * 6;
		}
	...
	case PUNCH_000_D:
		...
		// パンチの動き
		if (m_nMovingCounter[1] >= 60 * 0 && m_nMovingCounter[1] < 60 * 12) // 0秒から12秒の間は
		{
			// 動かない
		}
		if (m_nMovingCounter[1] >= 60 * 12 && m_nMovingCounter[1] < 60 * 14) // 12秒から14秒の間は
		{
			// 少し勢いをつけるように動く
			m_pos.x -= 0.1f;
			m_pos.z -= 0.1f;
		}
		if (m_nMovingCounter[1] >= 60 * 14 && m_nMovingCounter[1] < 60 * 15 - 50) // 14秒から15秒と5/6秒の間は
		{
			// 一気に進む
			m_pos.x += 30.0f;
			m_pos.z += 30.0f;
		}
		if (m_nMovingCounter[1] >= 60 * 15 - 50 && m_nMovingCounter[1] < 60 * 16) // 15秒と5/6秒から16秒の間は
		{
			// 動かない
		}
		if (m_nMovingCounter[1] >= 60 * 16) // 16秒に
		{
			// ワープして戻る
			m_pos = D3DXVECTOR3(-350.0f, 0.0f, -250.0f);
...
// 位置の設定処理
//----------------------------------------
void CPunch::SetPosition(D3DXVECTOR3 pos)
{
	m_pos = pos;
}
...
// 前回の位置の設定処理
//----------------------------------------
void CPunch::SetPositionOld(D3DXVECTOR3 posOld)
{
	m_posOld = posOld;
}
...
// 向きの設定処理
//----------------------------------------
void CPunch::SetRotation(D3DXVECTOR3 rot)
{
	m_rot = rot;
}
...
// サイズの設定処理
//----------------------------------------
void CPunch::SetSize(D3DXVECTOR3 size)
{
	m_size = size;
}
...
// 位置の取得処理
//----------------------------------------
D3DXVECTOR3 CPunch::GetPos(void)
{
	return m_pos;
}
...
// 前回の位置の取得処理
//----------------------------------------
D3DXVECTOR3 CPunch::GetPosOld(void)
{
	return m_posOld;
}
...
// 向きの取得処理
//----------------------------------------
D3DXVECTOR3 CPunch::GetRot(void)
{
	return m_rot;
}
...
// サイズの取得処理
//----------------------------------------
D3DXVECTOR3 CPunch::GetSize(void)
{
	return m_size;
}
